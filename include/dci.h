...
void data_projection(int num_indices, dci* const dci_inst, const int dim, const int num_points, const float* const data, float** p_data_proj, const bool* mask, bool pre_computed);

void dci_init(dci* const dci_inst, const int dim, const int num_comp_indices, const int num_simp_indices, float promotion_prob, float promotion_prob_subseq, int max_volume, bool transform, int parallel_level, bool debug, float* proj_vec);

// Note: the data itself is not kept in the index and must be kept in-place
long long dci_add(dci* const dci_inst, const int dim, const int num_points, const float* const data, const float* const value, const int num_levels, dci_query_config construction_query_config, const long long* const data_ids, int target_level, float* new_data_proj, bool* mask, bool random, int interval, int X, float anchor_threshold);
...
void dci_query(dci* const dci_inst, const int dim, const int num_queries, const float* const query, int num_neighbours, dci_query_config query_config, bool* mask, int** const nearest_neighbours, float** const nearest_neighbour_dists, int* const num_returned);
...
long long dci_add_query(dci* const dci_inst, const int dim, const int num_points, const float* const data, const float* const value, const int num_levels,
    ...
    const int num_queries, const float* const query, int num_neighbours, dci_query_config query_config, bool* delete_mask,
...
// Helper functions
void pack_points_to_matrix(additional_info** upper_cells, int num_points, int dim, float* points_matrix);
void compute_points_sq_norms(const float* points_matrix, int num_points, int dim, float* sq_norms);
