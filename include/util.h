...
// BLAS-accelerated (fastest!)
void compute_distances_matrix_blas(const float* query, const float* points_matrix,
    int num_points, int dim, const float* points_sq_norms, float* distances);

void compute_distances_transform_blas(const float* query, const float* points_matrix,
    int num_points, int dim, float max_sq_norm, float query_sq_norm,
    const float* points_sq_norms, float* distances);

// Portable fallback
void compute_distances_matrix_portable(const float* query, const float* points_matrix,
