...
{
    for (int i = 0; i < num_points; i++) {
        const float* point = points_matrix + i * dim;

        __m256 acc = _mm256_setzero_ps();
        int j = 0;

        for (; j + 7 < dim; j += 8) {
            __m256 p = _mm256_loadu_ps(point + j);
            acc = _mm256_fmadd_ps(p, p, acc);
        }

        float tmp[8];
        _mm256_storeu_ps(tmp, acc);
        float norm = tmp[0] + tmp[1] + tmp[2] + tmp[3] +
...
void data_projection(int num_indices, dci* const dci_inst, const int dim, 
    const int num_points, const float* const data, float** p_data_proj, const bool* mask, bool pre_computed) {
    ...
    if (dci_inst->transform) {
        ...
        for (i = 0; i < num_points; i++) {
            if (mask == NULL || mask[i]) {
                ...
                dci_inst->sq_norm_list[cur_next_point_id + i] = temp_norm;
                if (dci_inst->max_sq_norm < temp_norm) {
                    dci_inst->max_sq_norm = temp_norm;
                    // // For Debugging
                    // if (cur_next_point_id > 0) {
                    //     printf("Warning: max_sq_norm is updated\n");
                    // }
                }
            }
        }
        ...
        for (i = 0; i < num_points; i++) {
            if (mask == NULL || mask[i]) {
                for (j = 0; j < num_indices; j++) {
                    data_proj[j+i*num_indices] = data_proj[j+i*num_indices] + sqrt((dci_inst->max_sq_norm) - (dci_inst->sq_norm_list[cur_next_point_id + i])) * (dci_inst->add_proj_vec[j]);
                }
            }
        }
    }
...
static void update_local_dist(int num_indices, additional_info* point, float max_sq_norm, float* add_proj_vec, float* sq_norm_list) {
    float o_term = sqrt((point->max_sq_norm) - (sq_norm_list[point->id]));
    float n_term = sqrt((max_sq_norm) - (sq_norm_list[point->id]));
    for (int k = 0; k < num_indices; k++) {
        point->local_dist[k] += (n_term - o_term) * (add_proj_vec[k]);
    }
    point->max_sq_norm = max_sq_norm;
}

static void update_max_sq_norm(int num_indices, additional_info* point, float max_sq_norm, float* add_proj_vec, float* sq_norm_list, int* token2nodeIndex, int* token2nodeOffset, int* num_leaf_nodes, Stack* stack, bool **page_status, int dim, bool update_addr, btree_p_leaf_node ***leaf_list, int* max_leaves) {
...
static void dci_insert_to_indices(dci* dci_inst, bool transform, int num_indices, additional_info* parent, additional_info* point, float max_sq_norm, float* add_proj_vec, float* sq_norm_list) {
...
static int dci_query_single_point(
    ...
    const float* const query, const float* const query_proj,
...
static int dci_query_single_point_(
    int num_comp_indices, int num_simp_indices, int dim, int num_levels,
    additional_info* root, int num_populated_levels, int num_neighbours,
    const float* const query, const float query_proj,
    dci_query_config query_config, idx_arr* const top_candidates, 
    bool cumu, dci* const dci_inst, long long query_id, int num_upper_points, additional_info** upper_level_cells_ret, 
    float* points_matrix, float* all_distances, float* points_sq_norms);
...
static void dci_assign_parent(
    ...
    const long long* selected_query_pos, const float* const query,
    ...
    float* points_matrix, float* points_sq_norms);
...
void construct_new_tree(dci* const dci_inst, additional_info** root, const int dim,
    ...
    const float* const data, const float* const value, int** p_num_points_on_level,
    ...
    *root = (additional_info*)malloc(sizeof(additional_info));
    ...
    for (i = (*actual_num_levels) - 2; i >= 0; i--) {
        ...
        if (num_points_on_level[i + 1] > 0) {
            ...
            float* points_matrix = NULL;
            float* points_sq_norms = NULL;
            
            if (cblas_enabled && num_upper_points > 50) {
                points_matrix = (float*)malloc(num_upper_points * dim * sizeof(float));
...
#pragma omp parallel for if(dci_inst->parallel_level >= 2) num_threads(adaptive_threads) schedule(dynamic, chunk_size)
                for (int point_idx = 0; point_idx < num_points_on_cur_levels; point_idx++) {
                    ...
                    if (point_idx % interval == 0) {
                        ...
                        const float* cur_point_data = &data[cur_point_id * dim];
                        ...
                        for (int k = 0; k < unique_parent_count; k++) {
                            const float* parent_data = unique_parents[k]->data_loc;
...
int dci_delete(dci* const dci_inst, const int num_points, const long long* const data_ids, 
    ...
    float* temp_data_loc = NULL;
    float* temp_inc_data_loc = NULL;
    int dim = dci_inst->dim;
                ...
                if (promote) {
                    ...
                    if (iii >= last_level) {  // delete the promote data from its old parent
                        ...
                        if (promote_point->parent_info != NULL) {
                            if (update_addr) {
                                temp_data_loc = (float*)malloc(sizeof(float) * dim);
                                temp_inc_data_loc = (float*)malloc(sizeof(float) * dim);
...
long long dci_add_one_point(dci* const dci_inst, const int dim, const int num_points,
    const float* const data, const float* const value, dci_query_config construction_query_config, 
    ...
    if (dci_inst->num_points == 0) {
        ...
        if ((i == 0) && (interval > 0) && (num_points_on_cur_levels > dci_inst->numa_threshold)) {
            ...
            const float* cur_point_data = data;
...
long long dci_add(dci* const dci_inst, const int dim, const int num_points,
    const float* const data, const float* const value, const int num_levels, dci_query_config construction_query_config,
    ...
    if (num_upper_points <= dci_inst->numa_threshold) {
        ...
        if (cblas_enabled && points_matrix != NULL && all_distances != NULL && points_sq_norms != NULL) {
            ...
            for (int upper_idx = 0; upper_idx < num_upper_points; upper_idx++) {
                ...
                const float* upper_point_data = upper_cell->data_loc;
...
long long dci_add_query(dci* const dci_inst, const int dim,
    const int num_points, const float* const data, const float* const value, const int num_levels,
    ...
    for (int i = 0; i < update_num; i++) {
        ...
        if (leaf->data_loc == NULL) {
            for (int j = 0; j < leaf->num_slots_used; j++) {
                memcpy(fptr + (j * dci_inst->dim), leaf->slot_data[j].info->data_loc, (dci_inst->dim) * sizeof(float));
                memcpy(fptr + offset + (j * dci_inst->dim), leaf->slot_data[j].info->inc_data_loc, (dci_inst->dim) * sizeof(float));
            }
        }
        else {
            // // For Testing
            ...
            // }
            memcpy(fptr, leaf->data_loc, (leaf->num_slots_used) * (dci_inst->dim) * sizeof(float));
            memcpy(fptr + offset, leaf->inc_data_loc, (leaf->num_slots_used) * (dci_inst->dim) * sizeof(float));
            free(leaf->data_loc);
