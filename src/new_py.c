...
static PyObject *py_dci_new(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "iiidi", &dim, &num_comp_indices, &num_simp_indices, &lambda, &max_volume)) return NULL;
...
static PyObject *py_dci_add(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "OO!ibiiddi", &py_dci_inst_wrapper, &PyArray_Type, &py_data, &num_levels, &blind, &num_to_visit, &num_to_retrieve, &prop_to_visit, &prop_to_retrieve, &field_of_view)) return NULL;
    ...
    if (num_new_points > 0) {
        ...
        long long first_id = dci_add(&(py_dci_inst->dci_inst), dim, num_new_points, data, num_levels, construction_query_config);
