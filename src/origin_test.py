...
def gen_data():
    # load dataset
    f = h5py.File("fashion-mnist-784-euclidean.hdf5", "r")    # mode = {'w', 'r', 'a'}
    print("###### Load Dataset ######")
    print(f.filename, ":")
    print([key for key in f.keys()], "\n")
    train_data = np.array(f["train"], dtype=np.float64)  # (60000, 784)
    test_data = np.array(f["test"], dtype=np.float64)  # (10000, 784)
    distance = np.array(f["distances"], dtype=np.float64) # (10000, 100)
    neighbors = np.array(f["neighbors"], dtype=np.float64) # (10000, 100)

    return train_data, test_data, distance, neighbors
...
def main(*args):
    ...
    dim = 784              # Dimensionality of data
    intrinsic_dim = 50      # Intrinsic dimensionality of data
    num_points = 60000      # Number of data points
    ...
    t0 = time()
    data, query, dist, neighbor = gen_data()
    ...
    #                                   queries. 
    nearest_neighbour_idx, nearest_neighbour_dists = dci_db.query(query[:100], num_neighbours = num_neighbours, field_of_view = query_field_of_view, prop_to_retrieve = query_prop_to_retrieve)
