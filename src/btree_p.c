...
static float *btree_p_copy_emb_fwd(float *first, float *last, float *d_first);

static float *btree_p_copy_emb_bwd(float *first, float *last, float *d_last);
...
bool btree_p_insert(btree_p *const tree, const float key, const data_pt value, int *token2nodeIndex, int *token2nodeOffset, int* num_leaf_nodes, Stack *stack, bool **page_status, int dim, bool update_addr, btree_p_leaf_node ***leaf_list, int* max_leaves) {
    ...
    if (tree->root == NULL) {
        ...
        if (tree->first_leaf->data_loc == NULL && update_addr) {
            ...
            tree->first_leaf->data_loc = malloc(sizeof(float) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
            tree->first_leaf->inc_data_loc = malloc(sizeof(float) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
        }
...
btree_p_insert_descend(btree_p *const tree, void *n, const float key, const data_pt value, float *splitkey,
    ...
    } else // n->btree_p_is_leaf() == true
    {
        ...
        if (update_addr) {
            ...
            memcpy(leaf->data_loc + (slot * dim), value.info->data_loc, dim * sizeof(float));
            memcpy(leaf->inc_data_loc + (slot * dim), value.info->inc_data_loc, dim * sizeof(float));
            // for (int d = 0; d < dim; d++) {
...
static void btree_p_split_leaf(btree_p *const tree, btree_p_leaf_node *leaf, float *_newkey, void **_newleaf, 
    ...
    if (newleaf->data_loc == NULL && update_addr) {
        // Allocate a new space for the new leaf node
        newleaf->data_loc = malloc(sizeof(float) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
        newleaf->inc_data_loc = malloc(sizeof(float) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
    }
...
void btree_p_bulk_load(btree_p *const tree, float *keybegin, float *keyend, data_pt *databegin, data_pt *dataend, int *token2nodeIndex, int *token2nodeOffset, int *num_leaf_nodes, bool update_node, Stack *stack, bool **page_status, int dim, bool update_addr, btree_p_leaf_node ***leaf_list, int* max_leaves) {
    ...
    float* temp_data_loc;
    float* temp_inc_data_loc;
    if (update_node && update_addr) {
        temp_data_loc = (float *)malloc(sizeof(float) * num_items * dim);
        temp_inc_data_loc = (float *)malloc(sizeof(float) * num_items * dim);
        for (size_t i = 0; i < num_items; i++) {
            memcpy(temp_data_loc + (i) * dim, dt[i].info->data_loc, dim * sizeof(float));
            memcpy(temp_inc_data_loc + (i * dim), dt[i].info->inc_data_loc, dim * sizeof(float));
        }
    }
    ...
    for (size_t i = 0; i < num_leaves; ++i) {
        ...
        if (update_node) {
            ...
            if (leaf->data_loc == NULL && update_addr) {
                // Allocate a new space for the new leaf node
                leaf->data_loc = malloc(sizeof(float) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
                leaf->inc_data_loc = malloc(sizeof(float) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
            }
    ...
    if (update_node && update_addr) {
        ...
        for (size_t i = 0; i < num_leaves; ++i) {
            memcpy(leaf->data_loc, temp_data_loc + (cumulative_slots * dim), leaf->num_slots_used * dim * sizeof(float));
            memcpy(leaf->inc_data_loc, temp_inc_data_loc + (cumulative_slots * dim), leaf->num_slots_used * dim * sizeof(float));
