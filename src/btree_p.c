...
btree_p_insert_descend(btree_p *const tree, void *n, const float key, data_pt const value, float *splitkey,
                       void **splitnode, int *token2nodeIndex, int *token2nodeOffset, int* num_leaf_nodes, Stack *stack, bool **page_status, int dim, bool update_addr, btree_p_leaf_node ***leaf_list, int* max_leaves);

static void btree_p_split_leaf(btree_p *const tree, btree_p_leaf_node *leaf, float *_newkey, void **_newleaf, 
                               int *token2nodeIndex, int *token2nodeOffset, int* num_leaf_nodes, Stack *stack, bool **page_status, int dim, bool update_addr, btree_p_leaf_node ***leaf_list, int* max_leaves);
...
static void btree_p_free_node(void *n, unsigned int* num_leaf_nodes, Stack* stack);
...
static bf16_t *btree_p_copy_emb_fwd(bf16_t *first, bf16_t *last, bf16_t *d_first);

static bf16_t *btree_p_copy_emb_bwd(bf16_t *first, bf16_t *last, bf16_t *d_last);
...
btree_p_delete_descend(btree_p *const tree, const float key, const long long value, void *curr, void *left,
                       ...
                       btree_p_inner_node *parent, unsigned int parentslot, int *token2nodeIndex, int *token2nodeOffset, int* num_leaf_nodes, Stack *stack, bool *page_status, int dim, bool update_addr);

static btree_c_result btree_p_merge_leaves(btree_p *const tree, btree_p_leaf_node *left, btree_p_leaf_node *right,
                                           btree_p_inner_node *parent, int *token2nodeIndex, int *token2nodeOffset, int* num_leaf_nodes, Stack *stack, bool *page_status, int dim, bool update_addr);
...
btree_p_shift_left_leaf(btree_p_leaf_node *left, btree_p_leaf_node *right, btree_p_inner_node *parent,
                        unsigned int parentslot, int *token2nodeIndex, int *token2nodeOffset, int* num_leaf_nodes, Stack *stack, bool *page_status, int dim, bool update_addr);
...
static void btree_p_free_node(void *n, unsigned int* num_leaf_nodes, Stack* stack) {
    if (btree_p_is_leaf(n)) {
        btree_p_leaf_node *ln = (btree_p_leaf_node *) (n);
        if (ln->id >= 0) {
            ln->prev_leaf = ln->next_leaf = NULL;
            ln->level = 0;
            ln->num_slots_used = 0;
            push(stack, ln);
        }
...
static void clear_recursive(void *n, int* num_leaf_nodes, Stack* stack) {
    if (btree_p_is_leaf(n)) {
        ...
        for (unsigned short slot = 0; slot < innernode->num_slots_used + 1; ++slot) {
            clear_recursive(innernode->slot_ptrs[slot], num_leaf_nodes, stack);
            btree_p_free_node(innernode->slot_ptrs[slot], NULL, stack);
        }
    }
}
...
void btree_p_clear(btree_p *tree, int* num_leaf_nodes, Stack* stack) {
    if (tree->root) {
        clear_recursive(tree->root, num_leaf_nodes, stack);
        btree_p_free_node(tree->root, NULL, stack);
...
// Copy embedding in forward order
static bf16_t *btree_p_copy_emb_fwd(bf16_t *first, bf16_t *last, bf16_t *d_first) {
    while (first != last) {
        *d_first++ = *first++;
    }
...
// Copy embedding in reverse order
static bf16_t *btree_p_copy_emb_bwd(bf16_t *first, bf16_t *last, bf16_t *d_last) {
    while (first != last) {
        *(--d_last) = *(--last);
    }
...
bool btree_p_insert(btree_p *const tree, const float key, const data_pt value, int *token2nodeIndex, int *token2nodeOffset, int* num_leaf_nodes, Stack *stack, bool **page_status, int dim, bool update_addr, btree_p_leaf_node ***leaf_list, int* max_leaves) {
    ...
    if (tree->root == NULL) {
        ...
        btree_p_leaf_node * item = pop(stack);
        if (item != NULL)
            tree->root = tree->first_leaf = tree->last_leaf = item;
        else {
            tree->root = tree->first_leaf = tree->last_leaf = btree_p_allocate_leaf();
            tree->first_leaf->id = *num_leaf_nodes;
            // tree->first_leaf->data_loc = NULL;
            *num_leaf_nodes += 1;
            if (*num_leaf_nodes >= *max_leaves) {
                *max_leaves *= 2;
                *leaf_list = (btree_p_leaf_node **)realloc(*leaf_list, sizeof(btree_p_leaf_node *) * (*max_leaves));
                *page_status = (bool *)realloc(*page_status, sizeof(bool) * (*max_leaves));
            }
            (*leaf_list)[tree->first_leaf->id] = tree->first_leaf;
        }
        tree->num_leaf_nodes += 1;
        assert(tree->num_leaf_nodes == 1);
        
        if (tree->first_leaf->data_loc == NULL && update_addr) {
            // Allocate a new space for the new leaf node
            tree->first_leaf->data_loc = malloc(sizeof(bf16_t) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
            tree->first_leaf->inc_data_loc = malloc(sizeof(bf16_t) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
        }
        (*page_status)[tree->first_leaf->id] = 1;
    }

    bool r = btree_p_insert_descend(tree, tree->root, key, value, &newkey, &newchild, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr, leaf_list, max_leaves);
...
btree_p_insert_descend(btree_p *const tree, void *n, const float key, const data_pt value, float *splitkey,
    ...
    if (!btree_p_is_leaf(n)) {
        ...
        bool r = btree_p_insert_descend(tree, inner->slot_ptrs[slot],
                                        key, value, &newkey, &newchild, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr, leaf_list, max_leaves);
    ...
    } else // n->btree_p_is_leaf() == true
    {
        ...
        if (btree_p_is_full(tree, leaf)) {
            btree_p_split_leaf(tree, leaf, splitkey, splitnode, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr, leaf_list, max_leaves);
        ...
        if (update_addr) {
            btree_p_copy_emb_bwd(leaf->data_loc + (slot * dim), leaf->data_loc + (leaf->num_slots_used * dim),
                                leaf->data_loc + ((leaf->num_slots_used + 1) * dim));
            btree_p_copy_emb_bwd(leaf->inc_data_loc + (slot * dim), leaf->inc_data_loc + (leaf->num_slots_used * dim),
                                leaf->inc_data_loc + ((leaf->num_slots_used + 1) * dim));
            memcpy(leaf->data_loc + (slot * dim), value.info->data_loc, dim * sizeof(bf16_t));
            memcpy(leaf->inc_data_loc + (slot * dim), value.info->inc_data_loc, dim * sizeof(bf16_t));
            // for (int d = 0; d < dim; d++) {
            //     leaf->data_loc[slot * dim + d] = value.info->data_loc[d];
            //     leaf->inc_data_loc[slot * dim + d] = value.info->inc_data_loc[d];
            // }
        }
        leaf->slot_keys[slot] = key;
        leaf->slot_data[slot] = value;

        int node_index = leaf->id;
        assert(node_index >= 0);
        for (int i = slot; i < leaf->num_slots_used + 1; i++) {
            long long id = leaf->slot_data[i].info->id;
            if (i == slot)
                token2nodeIndex[id] = node_index;
            else
                assert(token2nodeIndex[id] == node_index);
            token2nodeOffset[id] = i;
            if (update_addr) {
                leaf->slot_data[i].info->data_loc = leaf->data_loc + (i * dim);
                leaf->slot_data[i].info->inc_data_loc = leaf->inc_data_loc + (i * dim);
            }
        }
        (*page_status)[node_index] = 1;
...
static void btree_p_split_leaf(btree_p *const tree, btree_p_leaf_node *leaf, float *_newkey, void **_newleaf, 
    ...
    btree_p_leaf_node *newleaf;
    btree_p_leaf_node * item = pop(stack);
    if (item != NULL)
        newleaf = item;
    else {
        newleaf = btree_p_allocate_leaf();
        newleaf->id = *num_leaf_nodes;
        // newleaf->data_loc = NULL;
        *num_leaf_nodes += 1;
        if (*num_leaf_nodes >= *max_leaves) {
            *max_leaves *= 2;
            *leaf_list = (btree_p_leaf_node **)realloc(*leaf_list, sizeof(btree_p_leaf_node *) * (*max_leaves));
        ...
        (*leaf_list)[newleaf->id] = newleaf;
    }
    if (newleaf->data_loc == NULL && update_addr) {
        // Allocate a new space for the new leaf node
        newleaf->data_loc = malloc(sizeof(bf16_t) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
        newleaf->inc_data_loc = malloc(sizeof(bf16_t) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
    }
    (*page_status)[newleaf->id] = 1;
    ...
    btree_p_copy_data_fwd(leaf->slot_data + mid, leaf->slot_data + leaf->num_slots_used,
                          newleaf->slot_data);
    if (update_addr) {
        btree_p_copy_emb_fwd(leaf->data_loc + (mid * dim), leaf->data_loc + (leaf->num_slots_used * dim),
                            newleaf->data_loc);
        btree_p_copy_emb_fwd(leaf->inc_data_loc + (mid * dim), leaf->inc_data_loc + (leaf->num_slots_used * dim),
                            newleaf->inc_data_loc);
        // float *tmp_end = leaf->inc_data_loc + (leaf->num_slots_used * dim);
        // float *tmp_data_p = leaf->inc_data_loc + (mid * dim);
        // while (tmp_data_p != tmp_end) {
        //     *tmp_data_p++ = 0;
        // }
    }
    for (int i = 0; i < newleaf->num_slots_used; i++) {
        long long id = newleaf->slot_data[i].info->id;
        token2nodeIndex[id] = newleaf->id;
        token2nodeOffset[id] = i;
        if (update_addr) {
            newleaf->slot_data[i].info->data_loc = newleaf->data_loc + (i * dim);
            newleaf->slot_data[i].info->inc_data_loc = newleaf->inc_data_loc + (i * dim);
        }
    }
    (*page_status)[newleaf->id] = 1;
...
void btree_p_bulk_load(btree_p *const tree, float *keybegin, float *keyend, data_pt *databegin, data_pt *dataend, int *token2nodeIndex, int *token2nodeOffset, int *num_leaf_nodes, bool update_node, Stack *stack, bool **page_status, int dim, bool update_addr, btree_p_leaf_node ***leaf_list, int* max_leaves) {
    ...
    bf16_t* temp_data_loc;
    bf16_t* temp_inc_data_loc;
    if (update_node && update_addr) {
        temp_data_loc = (bf16_t *)malloc(sizeof(bf16_t) * num_items * dim);
        temp_inc_data_loc = (bf16_t *)malloc(sizeof(bf16_t) * num_items * dim);
        for (size_t i = 0; i < num_items; i++) {
            memcpy(temp_data_loc + (i) * dim, dt[i].info->data_loc, dim * sizeof(bf16_t));
            memcpy(temp_inc_data_loc + (i * dim), dt[i].info->inc_data_loc, dim * sizeof(bf16_t));
        }
    }

    for (size_t i = 0; i < num_leaves; ++i) {
        // allocate new leaf node
        btree_p_leaf_node *leaf;
        ...
        if (update_node) {
            ...
            if (item != NULL)
                ...
                leaf = btree_p_allocate_leaf();
                leaf->id = *num_leaf_nodes;
                // leaf->data_loc = NULL;
                *num_leaf_nodes += 1;
                if (*num_leaf_nodes >= *max_leaves) {
                    *max_leaves *= 2;
                ...
                (*leaf_list)[leaf->id] = leaf;
            }
            if (leaf->data_loc == NULL && update_addr) {
                // Allocate a new space for the new leaf node
                leaf->data_loc = malloc(sizeof(bf16_t) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
                leaf->inc_data_loc = malloc(sizeof(bf16_t) * dim * BTREE_LEAF_MAX_NUM_SLOTS);
            }
            (*page_status)[leaf->id] = 1;
        }
        else
            leaf = btree_p_allocate_leaf();
        ...
        if (tree->last_leaf != NULL) {  // Non-empty case
        ...
        if (update_node) {
            for (int ii = 0; ii < leaf->num_slots_used; ii++) {
                long long id = leaf->slot_data[ii].info->id;
                token2nodeIndex[id] = leaf->id;
                token2nodeOffset[id] = ii;
                if (update_addr) {
                    leaf->slot_data[ii].info->data_loc = leaf->data_loc + (ii * dim);
                    leaf->slot_data[ii].info->inc_data_loc = leaf->inc_data_loc + (ii * dim);
                }
            }
            (*page_status)[leaf->id] = 1;
        }
    ...
    if (update_node && update_addr) {
        ...
        for (size_t i = 0; i < num_leaves; ++i) {
            memcpy(leaf->data_loc, temp_data_loc + (cumulative_slots * dim), leaf->num_slots_used * dim * sizeof(bf16_t));
            memcpy(leaf->inc_data_loc, temp_inc_data_loc + (cumulative_slots * dim), leaf->num_slots_used * dim * sizeof(bf16_t));
            cumulative_slots += leaf->num_slots_used;
            leaf = leaf->next_leaf;
        }
    }
    if (update_node && update_addr) {
...
// Node is considered a match if its key is within the range between key-(1e-3) and 
// key+(1e-3) inclusive and its ID is exactly equal to value. 
static int btree_p_delete_find_match(const btree_p_leaf_node *n, const float key, const long long value) {
    float keylow = key - 1e-3;
    float keyhi = key + 1e-3;
    int lo = 0;
    if (sizeof(n->slot_keys) > BTREE_BIN_SEARCH_THRESH) {
        if (n->num_slots_used == 0) return 0;
...
bool btree_p_delete(btree_p *const tree, const float key, const long long value, int *token2nodeIndex, int *token2nodeOffset, int* num_leaf_nodes, Stack *stack, bool *page_status, int dim, bool update_addr) {
    ...
    btree_c_result result = btree_p_delete_descend(tree, key, value, tree->root, NULL, NULL, NULL, NULL, NULL, 0, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr);
...
btree_p_delete_descend(btree_p *const tree, const float key, const long long value, void *curr, void *left,
    ...
    if (btree_p_is_leaf(curr)) {
        ...
        btree_p_copy_data_fwd(leaf->slot_data + slot + 1, leaf->slot_data + leaf->num_slots_used,
                              leaf->slot_data + slot);
        if (update_addr) {
            btree_p_copy_emb_fwd(leaf->data_loc + ((slot + 1) * dim), leaf->data_loc + (leaf->num_slots_used * dim),
                                leaf->data_loc+ (slot * dim));
            btree_p_copy_emb_fwd(leaf->inc_data_loc + ((slot + 1) * dim), leaf->inc_data_loc + (leaf->num_slots_used * dim),
                                leaf->inc_data_loc+ (slot * dim));
            // float *tmp_end = leaf->inc_data_loc + (leaf->num_slots_used * dim);
            // float *tmp_data_p = leaf->inc_data_loc + ((leaf->num_slots_used - 1) * dim);
            // while (tmp_data_p != tmp_end) {
            //     *tmp_data_p++ = 0;
            // }
        }
        ...
        if (btree_p_is_below_min(tree, leaf) && !(leaf == tree->root && leaf->num_slots_used >= 1)) {
            ...
            if (leftleaf == NULL && rightleaf == NULL) {
                ...
                if (leftparent == parent)
                    btree_p_delete_merge_result(&myres, btree_p_merge_leaves(tree, leftleaf, leaf, leftparent, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr));
                else
                    btree_p_delete_merge_result(&myres, btree_p_merge_leaves(tree, leaf, rightleaf, rightparent, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr));
            }
                ...
                if (rightparent == parent)
                    btree_p_delete_merge_result(&myres,
                                                btree_p_shift_left_leaf(leaf, rightleaf, rightparent, parentslot, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr));
                else
                    btree_p_delete_merge_result(&myres, btree_p_merge_leaves(tree, leftleaf, leaf, leftparent, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr));
            }
                // case : the left leaf has extra data, so balance left with current
                     ...
                     (rightleaf != NULL && btree_p_is_few(tree, rightleaf))) {
                if (leftparent == parent)
                    btree_p_shift_right_leaf(leftleaf, leaf, leftparent, parentslot - 1, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr);
                else
                    btree_p_delete_merge_result(&myres, btree_p_merge_leaves(tree, leaf, rightleaf, rightparent, token2nodeIndex, token2nodeOffset, num_leaf_nodes, stack, page_status, dim, update_addr));
            }
    ...
    } else // !curr->btree_p_is_leaf()
    {
        ...
        int slot = btree_p_find_lower_inner(inner, key - 1e-3);
        ...
        while (slot <= inner->num_slots_used) {
            ...
            result = btree_p_delete_descend(tree, key, value,
                                            ...
                                            inner, slot, token2nodeIndex, 
                                            token2nodeOffset, num_leaf_nodes,
                                            stack, page_status, dim, update_addr);
            ...
            if (slot < inner->num_slots_used && inner->slot_keys[slot] > key + 1e-3)
                return btree_c_result_of(btree_not_found, 0);
        ...
        if (btree_p_result_has(result, btree_fix_merge)) {
            ...
            btree_p_free_node(inner->slot_ptrs[slot], &(tree->num_leaf_nodes), stack);
        ...
        if (btree_p_is_below_min(tree, inner) && !(inner == tree->root && inner->num_slots_used >= 1)) {
            ...
            if (leftinner == NULL && rightinner == NULL) {
                ...
                btree_p_free_node(inner, &(tree->num_leaf_nodes), stack);
...
static btree_c_result btree_p_merge_leaves(btree_p *const tree, btree_p_leaf_node *left, btree_p_leaf_node *right,
    ...
    if (update_addr) {
        btree_p_copy_emb_fwd(right->data_loc, right->data_loc + (right->num_slots_used * dim),
                            left->data_loc+ (left->num_slots_used * dim));
        btree_p_copy_emb_fwd(right->inc_data_loc, right->inc_data_loc + (right->num_slots_used * dim),
                            left->inc_data_loc+ (left->num_slots_used * dim));
        // float *tmp_end = right->inc_data_loc + (right->num_slots_used * dim);
        // float *tmp_data_p = right->inc_data_loc;
        // while (tmp_data_p != tmp_end) {
        //     *tmp_data_p++ = 0;
        // }
    }

    int node_index = left->id;
    assert(node_index >= 0);
    for (int i = left->num_slots_used; i < left->num_slots_used + right->num_slots_used; i++) {
        long long id = left->slot_data[i].info->id;
        token2nodeIndex[id] = node_index;
        token2nodeOffset[id] = i;
        if (update_addr) {
            left->slot_data[i].info->data_loc = left->data_loc + (i * dim);
            left->slot_data[i].info->inc_data_loc = left->inc_data_loc + (i * dim);
        }
    }
    page_status[node_index] = 1;
...
btree_p_shift_left_leaf(btree_p_leaf_node *left, btree_p_leaf_node *right, btree_p_inner_node *parent,
    ...
    btree_p_copy_data_fwd(right->slot_data, right->slot_data + shiftnum,
                          left->slot_data + left->num_slots_used);
    if (update_addr) {
        btree_p_copy_emb_fwd(right->data_loc, right->data_loc + (shiftnum * dim),
                            left->data_loc + (left->num_slots_used * dim));
        btree_p_copy_emb_fwd(right->inc_data_loc, right->inc_data_loc + (shiftnum * dim),
                            left->inc_data_loc + (left->num_slots_used * dim));
        // float *tmp_end = right->inc_data_loc + (shiftnum * dim);
        // float *tmp_data_p = right->inc_data_loc;
        // while (tmp_data_p != tmp_end) {
        //     *tmp_data_p++ = 0;
        // }
    ...
    int node_index = left->id;
    assert(node_index >= 0);
    for (int i = left->num_slots_used; i < left->num_slots_used + shiftnum; i++) {
        long long id = left->slot_data[i].info->id;
        token2nodeIndex[id] = node_index;
        token2nodeOffset[id] = i;
        if (update_addr) {
    ...
    btree_p_copy_data_fwd(right->slot_data + shiftnum, right->slot_data + right->num_slots_used,
                          right->slot_data);
    if (update_addr) {
        btree_p_copy_emb_fwd(right->data_loc + (shiftnum * dim), right->data_loc + (right->num_slots_used * dim),
                            right->data_loc);
        btree_p_copy_emb_fwd(right->inc_data_loc + (shiftnum * dim), right->inc_data_loc + (right->num_slots_used * dim),
                            right->inc_data_loc);
        // float *tmp_end = right->inc_data_loc + (right->num_slots_used * dim);
        // float *tmp_data_p = right->inc_data_loc + ((right->num_slots_used - shiftnum) * dim);
        // while (tmp_data_p != tmp_end) {
        //     *tmp_data_p++ = 0;
        // }
    }
    ...
    node_index = right->id;
    assert(node_index >= 0);
    for (int i = 0; i < right->num_slots_used - shiftnum; i++) {
        long long id = right->slot_data[i].info->id;
        assert(token2nodeIndex[id] == node_index);
        token2nodeOffset[id] = i;
        if (update_addr) {
            right->slot_data[i].info->data_loc = right->data_loc + (i * dim);
            right->slot_data[i].info->inc_data_loc = right->inc_data_loc + (i * dim);
        }
    }
    page_status[node_index] = 1;
...
static void btree_p_shift_right_leaf(btree_p_leaf_node *left, btree_p_leaf_node *right, btree_p_inner_node *parent,
    ...
    if (update_addr) {
        btree_p_copy_emb_bwd(right->data_loc, right->data_loc + (right->num_slots_used * dim),
                            right->data_loc + (right->num_slots_used + shiftnum) * dim);
        btree_p_copy_emb_bwd(right->inc_data_loc, right->inc_data_loc + (right->num_slots_used * dim),
                            right->inc_data_loc + (right->num_slots_used + shiftnum) * dim);
    }

    int node_index = right->id;
    assert(node_index >= 0);
    for (int i = shiftnum; i < right->num_slots_used + shiftnum; i++) {
        long long id = right->slot_data[i].info->id;
        assert(token2nodeIndex[id] == node_index);
        token2nodeOffset[id] = i;
        if (update_addr) {
    ...
    btree_p_copy_data_fwd(left->slot_data + left->num_slots_used - shiftnum, left->slot_data + left->num_slots_used,
                          right->slot_data);
    if (update_addr) {
        btree_p_copy_emb_fwd(left->data_loc + ((left->num_slots_used - shiftnum) * dim), left->data_loc + (left->num_slots_used * dim),
                            right->data_loc);
        btree_p_copy_emb_fwd(left->inc_data_loc + ((left->num_slots_used - shiftnum) * dim), left->inc_data_loc + (left->num_slots_used * dim),
                            right->inc_data_loc);
        // float *tmp_end = left->inc_data_loc + (left->num_slots_used * dim);
        // float *tmp_data_p = left->inc_data_loc + ((left->num_slots_used - shiftnum) * dim);
        // while (tmp_data_p != tmp_end) {
        //     *tmp_data_p++ = 0;
        // }
    }
